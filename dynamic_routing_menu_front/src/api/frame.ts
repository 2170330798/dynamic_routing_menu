...
export const getFrameData = async () => {
    ...
    try {
      ...
      if (request.data.code === 200) {
        ...
        //将响应数据存到store中
        store.setFrame(request.data.data);
        console.log('框架数据:', store.getIFrame);

      }else {
        // 处理code不是200的情况
        console.warn('获取框架数据成功，但返回状态码非200:', request.data);
        // 可以选择在这里抛出错误或进行其他处理
        throw new Error(request.data.message || '获取框架数据失败');
      }
        ...
        if (axios.isAxiosError(err)) {
              ...
              if (err.request) {
                ...
                console.error('获取框架失败，服务器返回错误:', err.request.status, err.request.data);
              } else if (err.request) {
                // 请求已发出，但没有收到响应
                console.error('获取框架失败，无服务器响应:', err.request);
              } else {
                // 请求设置出错
                console.error('获取框架失败，请求设置错误:', err.message);
              }
        } else {
              // 处理非Axios错误
              console.error('获取框架失败，发生意外错误:', err instanceof Error ? err.message : err);
        }
        console.error('获取框架失败:', err);
        // 可以选择将错误传递到上层或进行其他处理
        throw err;
...
export const addFrame = async(data : IFrame) => {

    
    try {
      const request = await axios.post(ADD_FRAME_REQUEST_API, data);
      if (request.data.code === 200){
          console.log('提交数据成功', request.data.code);
          console.log('提交数据', data);
          return '提交数据成功';
      }else if(request.data.code == 401){
        console.log('框架ID违规，请重新提交');
    }else {
...
export const deleteFrame = async(frameId: number) => {

  try {
    const request = await axios.delete(DELETE_FRAME_REQUEST_API+`?id=${frameId}`);
    if (request.data.code === 200){
...
export const updateFrame = async(data : IFrame) => {

  try {
    const request = await axios.put(UPDATE_FRAME_REQUEST_API, data);
    if (request.data.code === 200){
